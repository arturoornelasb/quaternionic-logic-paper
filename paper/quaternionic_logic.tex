...
Let $\varphi$ be a propositional formula. The following are equivalent:
\begin{enumerate}[nosep]
\item $\varphi$ is a G-lattice tautology: $\Real(\llbracket\varphi\rrbracket_v)\geq\tau$ for every $v:\mathrm{Atoms}\to V$.
\item $\varphi$ is a G\"odel tautology: $\llbracket\varphi\rrbracket_w\geq\tau$ for every $w:\mathrm{Atoms}\to[0,1]$.
\item $\varphi$ is derivable in the G\"odel--Dummett axiom system.
\end{enumerate}
\end{theorem}

\begin{proof}
$(1)\Rightarrow(2)$: Restrict to valuations with $\Imag=0$.
These are G\"odel valuations, so every G-lattice tautology is a
G\"odel tautology.

$(2)\Rightarrow(1)$: By Lemma~\ref{lem:re-indep},
$\Real(\llbracket\varphi\rrbracket_v)$ depends only on the real
parts of the atomic valuations. If $\varphi$ holds for all
$[0,1]$-valuations of these real parts, it holds for all G-lattice
valuations regardless of imaginary components.

$(2)\Leftrightarrow(3)$: Standard G\"odel--Dummett completeness
...
alter which formulas are tautologies. Completeness is inherited from
G\"odel logic via the real-part independence lemma. The ``new'' content
...
G\"odel tautologies (7 formulas) & 100\,000 & 0 & confirmed \\
Non-tautologies (3 formulas) & 100\,000 & 0 & confirmed \\
\bottomrule
\end{tabularx}
\end{table}
...
The G-lattice inherits completeness from G\"odel--Dummett logic
via the real-part independence lemma (Theorem~\ref{thm:completeness}).
The imaginary dimensions do not alter tautologies---they enrich
the surrounding structure.

\paragraph{Future work.}
Empirical validation on the 72 semantic primitives from
\citet{ornelas2026p9} is planned.
