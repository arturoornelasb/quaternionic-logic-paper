...
$V=[0,1]\times[-1,1]^3$, where the real component encodes truth
...
while the full four-dimensional logic is context-dependent. Boolean
and G\"odel fuzzy logic are recovered exactly as dimensional
restrictions; four additional logics (modal, trivalent, ordinal,
probabilistic) admit interpretive correspondences.
...
\item A bounded residuated lattice on $[0,1]\times[-1,1]^3$
  that recovers Boolean and G\"odel fuzzy logic exactly, with
  interpretive correspondences to four others
...
holds because $1-\min(x,y)=\max(1-x,1-y)$ for the real component and
...
\label{thm:recovery}
Under dimensional restrictions, the lattice $(V,\mathrm{AND},\mathrm{OR},\mathrm{NOT},\to)$ reduces exactly to:
\begin{enumerate}[nosep]
...
\begin{remark}[Interpretive correspondences]
\label{rem:interpretive}
Four additional logics admit \emph{interpretive correspondences}
(not formal isomorphisms) under dimensional restriction:
\begin{enumerate}[nosep,start=3]
\item \textbf{Modal}: $j=k=0$, $r,i\in[0,1]$, reading
  necessity as $r=i=1$ and possibility as $r>0\lor i>0$.
\item \textbf{Trivalent}: $i=k=0$, $j\in\{-1,0,+1\}$,
  encoding the third value via the sign of $j$.
\item \textbf{Ordinal}: $j=k=0$, $i$ encoding rank.
\item \textbf{Probabilistic}: $j=k=0$, $i$ encoding variance.
\end{enumerate}
These are suggestive readings of the restricted subspaces, not
claims that a modal or probabilistic logician would recognise
their formal system. The value of the quaternionic lattice rests
on the exact recovery of Boolean and G\"odel logic and the
commutation theorem (Theorem~\ref{thm:commutation}), not on
these looser analogies.
\end{remark}
...
\begin{remark}[Domain of the action]
\label{rem:domain}
Conjugation acts on $\Imag(a)$ as an $\mathrm{SO}(3)$ rotation,
preserving $\|\Imag\|$ but potentially redistributing components
beyond $[-1,1]$ when $\|\Imag(a)\|>1$. The action is therefore
defined on the ambient space $[0,1]\times\mathbb{R}^3\supseteq V$.
Every purely real element $(r,0,0,0)$ is a fixed point, so the
Boolean sublattice ($\Imag=0$) is always preserved. The logical
content of the G-lattice depends only on $\Real$ (by
Lemma~\ref{lem:re-indep}), so the domain extension does not
affect tautologies or derivability.
\end{remark}
